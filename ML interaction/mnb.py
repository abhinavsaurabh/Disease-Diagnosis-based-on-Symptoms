...
def MNB(X,Y,sample_x,df_norm,dataset_symptoms,final_symp,scores):
  mnbclf = MultinomialNB(alpha=1.0, fit_prior=True)
  mnbclf.fit(X,Y)
  prediction = mnbclf.predict_proba([sample_x])
  ...
  print(f"\nTop {k} diseases predicted based on symptoms by Multinomial Naive bayes")
  topk_dict = {}
