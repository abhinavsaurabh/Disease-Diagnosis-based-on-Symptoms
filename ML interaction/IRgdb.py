...
def GDB(X,Y,sample_x,df_norm,dataset_symptoms,final_symp,scores):
  gdbmod = GradientBoostingClassifier(learning_rate=0.1, n_estimators=50)
  gdbmod.fit(X,Y)
  prediction = gdbmod.predict_proba([sample_x])
  ...
  print(f"\nTop {k} diseases predicted based on symptoms by Gradient Boosting ")
  topk_dict = {}
