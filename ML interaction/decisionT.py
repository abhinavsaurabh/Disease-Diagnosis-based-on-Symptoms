...
def decisionT(X,Y,sample_x,df_norm,dataset_symptoms,final_symp,scores):
  dtree_mod=dtree(criterion='entropy')
  dtree_mod.fit(X,Y)
  prediction = dtree_mod.predict_proba([sample_x])
  ...
  print(f"\nTop {k} diseases predicted based on symptoms by Decision Tree")
  topk_dict = {}
