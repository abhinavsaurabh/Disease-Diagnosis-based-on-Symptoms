...
def SVM(X,Y,sample_x,df_norm,dataset_symptoms,final_symp,scores):
  svmmod=svm.SVC(kernel='linear', C=1, probability=True)
  svmmod.fit(X,Y)
  prediction = svmmod.predict_proba([sample_x])
  ...
  print(f"\nTop {k} diseases predicted based on symptoms by SVM")
  topk_dict = {}
