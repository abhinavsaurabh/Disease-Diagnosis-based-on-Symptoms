...
def RandF(X,Y,sample_x,df_norm,dataset_symptoms,final_symp,scores):
  rfmod=rf(n_estimators=20,criterion='entropy')
  rfmod.fit(X,Y)
  prediction = rfmod.predict_proba([sample_x])
  ...
  print(f"\nTop {k} diseases predicted based on symptoms by Random Forest")
  topk_dict = {}
